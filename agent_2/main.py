...
用法示例：
  # 单文件处理
  python main.py --input /Users/wwz/Documents/数据清洗/data process/agent_2-3/reorganized_output --output results/
...
async def _batch_mode(
    ...
    entries = sorted(
        ...
        if not entry.startswith(".") and entry not in {".DS_Store", "Thumbs.db"}
    )
    total = len(entries)
