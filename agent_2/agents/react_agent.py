...
class ReactMedicalAgent(AgentBase):
    ...
    async def _run_directory(self, dir_path: str, plan: ProcessingPlan) -> Dict[str, Any]:
        ...
        # 导出 CSV：合并宽表（表格列 + OCR 实体列，每患者一行）
        # 只要有任何患者有 table_data 就生成 merged
        has_any_table = any(
        ...
        if has_any_table:
            ...
            ocr_cols: list = []
            seen_ocr_cols: set = set()

            def _add_ocr_col(col: str) -> None:
                if col not in seen_ocr_cols:
                    seen_ocr_cols.add(col)
                    ocr_cols.append(col)
            ...
            for pid, pdata in all_results["patients"].items():
                ...
                for cat, ents in by_cat.items():
                    ...
                    for e in ents:
                        ...
                        v = e.get("value", "")
                        u = e.get("unit", "")
                        parts.append(" ".join(filter(None, [n, v, u])))
                    row[raw_col] = "; ".join(parts)
                    _add_ocr_col(raw_col)

                    seen_name: Dict[str, Dict] = {}
                    for e in ents:
                        seen_name[e.get("name", "")] = e
                    for name, e in seen_name.items():
                        v = e.get("value", "")
                        u = e.get("unit", "")
                        combined = " ".join(filter(None, [v, u]))
                        for col, val in [
                            (f"Extracted_{cat}_{name}", combined),
                            (f"Extracted_{cat}_{name}_value", v),
                            (f"Extracted_{cat}_{name}_unit", u),
                        ]:
                            row[col] = val
                            _add_ocr_col(col)
            ...
            # 取第一个有 table_data 的患者确定列顺序
            first_table_row = next(
                (p["table_data"] for p in all_results["patients"].values() if p.get("table_data")),
                {}
            )
            xlsx_cols = list(first_table_row.keys())
            out_cols = xlsx_cols + ocr_cols
            merged_rows = []
            for pid in sorted(ocr_rows.keys()):
                table_row = dict(all_results["patients"][pid].get("table_data", {}))
                table_row.update(ocr_rows[pid])
                merged_rows.append(table_row)
