...
def _should_ignore_entry(name: str) -> bool:
    return name.startswith(".") or name in _IGNORED_FILE_NAMES
            ...
            if cat.lower() == "table":
                for fname in sorted(os.listdir(cat_path)):
                    if _should_ignore_entry(fname):
                        continue
                    if fname.endswith(_table_exts):
                        patients[pid]["table_path"] = os.path.join(cat_path, fname)
                        if table_path is None:
                            table_path = patients[pid]["table_path"]
                        break
                continue
