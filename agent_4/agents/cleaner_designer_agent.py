...
def _get_backend() -> str:
    backend = str(os.environ.get("STEP4_MODEL_BACKEND") or AGENT_CFG.get("backend") or "ollama").strip().lower()
...
def _get_openai_cfg() -> dict:
    ...
    return {
        "model_name": os.environ.get("STEP4_OPENAI_MODEL")
        or cfg.get("model_name")
        or OPENAI_MODEL_NAME,
        "api_key": os.environ.get("STEP4_OPENAI_API_KEY")
        or os.environ.get("OPENAI_API_KEY", ""),
        "api_base": os.environ.get("STEP4_OPENAI_API_BASE")
        or cfg.get("api_base")
        or os.environ.get("OPENAI_API_BASE")
        or OPENAI_API_BASE,
        "timeout": int(
            os.environ.get("STEP4_OPENAI_TIMEOUT")
            or cfg.get("timeout")
            or os.environ.get("OPENAI_TIMEOUT")
            or OPENAI_TIMEOUT
        ),
        "temperature": float(
            os.environ.get("STEP4_OPENAI_TEMPERATURE")
            or cfg.get("temperature")
...
def _get_model_and_formatter():
    ...
    if backend == "openai":
        cfg = _get_openai_cfg()
        if not cfg["api_key"]:
            raise RuntimeError("Step4 配置为 OpenAI 后端，但未设置 STEP4_OPENAI_API_KEY 或 OPENAI_API_KEY。")
