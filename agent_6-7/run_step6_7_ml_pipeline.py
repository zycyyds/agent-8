...
数据来源
────────
  data/副本all_patients.csv                     原始数据（多列，多患者）
  data/副本all_patients_filtered_*.csv          筛选后数据

输出
────
  output_step6/consistency_report_<ts>.txt/.json
  output_step7/ml_dataset_<ts>.csv/.jsonl/.json
...
def _get_auto_feature_cols() -> list[str]:
    ...
    try:
        with open(SELECTION_REPORT, encoding="utf-8") as f:
            report = json.load(f)
        final_cols = report.get("final_columns", [])
    except Exception:
...
def load_task_context() -> ToolResponse:
    ...
    task_text  = report.get("task_text", "N/A")
    final_cols = report.get("final_columns", [])
    ...
    diag_cols = [c for c in final_cols if "诊断" in c and not _should_skip(c)]
    lines += [
        "",
...
def format_and_save_ml_dataset(
    ...
    **粒度：筛选后 CSV 的每一行 = 一条 ML 样本**（同一患者多条检查/多条记录
    ...
    pytorch = {
        ...
        "task_text":      sel_report.get("task_text", ""),
...
async def run_step6_only(runtime_overrides: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    msg6 = Msg(
        ...
        content=(
            ...
            "完成后请保存报告，并在末尾标注 PASSED_PATIENTS: [...]。"
        ),
    )
...
async def run_step7_only(
    ...
    msg7 = Msg(
        ...
        content=(
            "Step 6 一致性验证已完成。\n\n"
            f"通过验证的患者 ID: {_passed_patient_ids}\n\n"
            "请执行 Step 7：基于任务目标生成 ML 训练数据集。\n\n"
            "重要提示：ICD 标准化使用本地中文 ICD-10 库（35,725 条），无需翻译和外部 API。\n"
            + _step7_diag_hint
            + "请务必先调用 discover_diagnosis_fields() 识别所有含诊断信息的字段，\n"
            "再调用 extract_and_map_all_diagnosis_fields() 完成多字段 ICD 标准化，\n"
            "然后调用 propose_label_mapping(label_column=..., mode=\"multiclass_enum\") "
            "自动生成标签映射（不要向用户追问 label_mapping），\n"
            "再依次调用 build_label_series 与 format_and_save_ml_dataset 完成保存。\n\n"
            f"筛选后数据路径：{FILTERED_CSV}\n"
            f"列筛选报告路径：{SELECTION_REPORT}"
        ),
    )
